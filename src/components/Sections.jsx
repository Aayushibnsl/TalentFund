...
const Sections = () => {
  ...
  const applyStudentButtonRef = useRef(null);
  const becomeInvestorButtonRef = useRef(null);
  ...
  useEffect(() => {
    const handleApplyStudentClick = () => {
      window.location.href = "userform.html";
    };

    const applyStudentButton = applyStudentButtonRef.current;
    if (applyStudentButton) {
      applyStudentButton.addEventListener("click", handleApplyStudentClick);
    }

    return () => {
      if (applyStudentButton) {
        applyStudentButton.removeEventListener("click", handleApplyStudentClick);
      }
    };
  }, []);

  useEffect(() => {
    const handleBecomeInvestorClick = () => {
      window.location.href = "investor_form.html";
    };

    const becomeInvestorButton = becomeInvestorButtonRef.current;
    if (becomeInvestorButton) {
      becomeInvestorButton.addEventListener("click", handleBecomeInvestorClick);
    }

    return () => {
      if (becomeInvestorButton) {
        becomeInvestorButton.removeEventListener("click", handleBecomeInvestorClick);
      }
    };
  }, []);
      ...
      <div className="container-custom">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            ...
            <a href="userform.html" className="btn-primary">Apply as Student</a>
          </div>
          ...
          >
            ...
            <a href="investor_form.html" className="btn-secondary">Apply as Investor</a>
          </div>
        </div>
      </div>
    </section>
